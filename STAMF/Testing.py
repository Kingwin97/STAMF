...
def test_net(args):
    ...
    model_path = args.save_model_dir + 'mayb-USODP-final-10w.pth'
    print(model_path)
    ...
    for test_dir_img in test_paths:
        ...
        for i, data_batch in enumerate(test_loader):
            ...
            # save saliency maps
            save_test_path = args.save_test_path_root + dataset + '/USOD/'
            if not os.path.exists(save_test_path):
                os.makedirs(save_test_path)
            cv2.imwrite(os.path.join(save_test_path, filename + '.png'),d1*255)
