...
def load_list(dataset_list, data_root):
    ...
    for dataset_name in dataset_list:
        ...
        depth_root = data_root + dataset_name + '/Polar/'
        depth_files = os.listdir(depth_root)

        for depth in depth_files:
            images.append(depth_root.replace('/Polar/', '/RGB/') + depth[:-4]+'.png')
            depths.append(depth_root + depth[:-4]+'.png')
            labels.append(depth_root.replace('/Polar/', '/GT/') + depth[:-4]+'.png')
            contours.append(depth_root.replace('/Polar/', '/edge/') + depth[:-4]+'_edge.png')
...
def load_test_list(test_path, data_root):
    ...
    depth_root = data_root + test_path + '/Polar/'
