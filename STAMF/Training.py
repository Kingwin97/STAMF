...
def main(local_rank, num_gpus, args):
    ...
    base_params = [params for name, params in net.named_parameters() if ("rgb_backbone" in name)]

    other_params = [params for name, params in net.named_parameters() if ("rgb_backbone" not in name)]
    ...
    for epoch in range(args.epochs):
        ...
        for i, data_batch in enumerate(train_loader):
            ...
            ##可视化loss
            writer.add_scalar('Loss/Total', total_loss.item(), epoch *len(train_loader) +i)
            writer.add_scalar('Loss/BCE', BCE_total_loss.item(), epoch * len(train_loader) + i)
            writer.add_scalar('Loss/IoU', IoU_total_loss.item(), epoch * len(train_loader) + i)
            writer.add_scalar('Loss/Edge', Edge_total_loss.item(), epoch * len(train_loader) + i)
            writer.add_scalar('Loss/Dice', Dice_total_loss.item(), epoch * len(train_loader) + i)
            ##可视化loss
            ...
            if whole_iter_num == args.train_steps // 4:
                torch.save(net.state_dict(), args.save_model_dir + 'UVST_half+half4.pth')
            elif whole_iter_num == 50000:
                torch.save(net.state_dict(), args.save_model_dir + 'UVST_half_5w.pth')
            elif whole_iter_num == args.train_steps//2:
                torch.save(net.state_dict(), args.save_model_dir + 'UVST_half2.pth')
            elif whole_iter_num == 70000:
                torch.save(net.state_dict(), args.save_model_dir + 'UVST_half_7w.pth')
            elif whole_iter_num == 80000:
                torch.save(net.state_dict(), args.save_model_dir + 'UVST_half_8w.pth')
            elif whole_iter_num == args.train_steps:
                torch.save(net.state_dict(), args.save_model_dir + 'UVST_final.pth')
